...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleCAPTCHARqRs("CaptchaView?stor");
            ...
            else if (Instruction == 3)
            {
                ...
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "pinNumber", txtCardPIN.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "captchaVerification", txtCAPTCHAAnswer.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%btn_Chk-GC-Balance.jpg","");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance(p1, "Enter", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
        ...
        private void DoMacro1()
        {
            ...
            string FileToUse = GCGCommon.PMC.WriteMacro(txtRqRsPath.Text, "" +
                "Pause,1000~!~" +
                "WinActivate,Balance Extractor - XXX~!~" +
                "Pause,1000~!~" +
                "Move,450,350~!~" +
                "LeftClick~!~" +
                "Pause,100~!~" +
