...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.InnerText, "Check Balance", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                //OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.InnerText, "Check Balance","");
                DoGCGDelay(20, true);
                if (OK == "1")
                {
                    /*
                    ...
                    DoWait(200)
                    DoTyping(X)
                    DoRightClick(486,431)
                    DoRightClick(486,431)
                    DoWait(500)
                    DoLeftClick(535,627)
                    */
                    PlayMacro("DoLeftClick(600,600)~!~DoWait(500)~!~DoTyping(% )~!~DoWait(200)~!~DoTyping(X)~!~DoWait(200)~!~DoRightClick(486,431)~!~DoRightClick(486,431)~!~DoWait(500)~!~DoLeftClick(535,627)"); //
                }
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                OK = DoHandleCAPTCHARqRs(0, 0, 0, 0);
                HandleInstruction(OK);
            }
            else if (Instruction == 5)
            {
                PlayMacro("DoLeftClick(519,346)~!~DoWait(250)~!~DoTyping(" + txtCardNumber.Text + "{TAB}" +txtCAPTCHAAnswer.Text + "{TAB}{ENTER})");
            }
            else
            {
                GCGMethods.KillProcess("iexplore.exe");
                //RetryCntMax = 5;
                string balanceResult = "";
                string test = GCGMethods.GetHTML(FrameDoc);
                test = GCGMethods.GetPlainTextFromHTML(test);
                if (test.Contains("Sorry"))
                {
                    txtCardBalance.Text = "Oops";
                    //SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCBALANCEERR.ToString(), "The number you entered was invalid.", ad.RsPathAndFileToWrite);
                    SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The number you entered was invalid.", ad.RsPathAndFileToWrite);
                ...
                if (balanceResult == "")
                {
                    balanceResult = GetBalance(l4 + " is", "Amount", test);
                }
                if (balanceResult == "")
                {
                    //SpecificRetryCnt++;
        ...
        */
        ...
        private void PlayMacro(string Script)
        {
            //MacroDel="~!~"
            //ExampleScript=DoLeftClick(960,781)~!~DoWait(500)~!~DoLeftClick(830,393)
            tmrRunning.Enabled = false;
            string temp = SupportMethods.CreateAlphaNumericKey(6) + ".txt";  //testscript
            string completetemp = ad.RxPath + "\\" + temp;
            string[] arr = Script.Split(new string[] { "~!~" },
            ...
            sw.Close();
            string GCGMacroPlayerLoc = Environment.CurrentDirectory + "\\GCGMacroPlayer.exe";
            if (!File.Exists(GCGMacroPlayerLoc)) GCGMacroPlayerLoc = "C:\\GCGMacroPlayer.exe";
            GCGCommon.SupportMethods.LaunchIt(GCGMacroPlayerLoc, completetemp);
            bool IsGCGMacroRunning=true;
            while (IsGCGMacroRunning==true)
            {
                if (!File.Exists(completetemp)) IsGCGMacroRunning = false;
                Application.DoEvents();
            }
            tmrRunning.Enabled = true;
        }
