...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            if (RetryCnt >= RetryCntMax || SpecificRetryCnt > 9)
            {
            ...
            else if (Instruction == 2)
            {
                ...
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.OuterHtml, "Check your balance", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, "kaptcha?");
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                IHTMLDocument2 FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gcBal_giftNumEntry", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gcBal_pinNumEntry", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gcBal_captchaEntry", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gcBal_checkSubmit", "");
            }
            else
            {
                string balanceResult = "";
                IHTMLDocument2 FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1);
                string test = GCGMethods.GetHTMLFromWebObject(FrameDoc);
                test = GCGMethods.GetPlainTextFromHTML(test);
                //string test = DoManualCopyAndPaste();
                //GCGMethods.WriteFile("C:\\GetBalance.txt", test, true);
                balanceResult = GetBalance("Your gift card balance is", ".Check", test);
                if (balanceResult == "")
                {
                    //Instruction--;
                    //SpecificRetryCnt++;
        ...
        private string DoDelay(int pDelayAmnt)
        {
        ...
        private string ChangeFocus(string SearchForInOuterHtml, GCGCommon.HTMLEnumTagNames htmltype, FocusTypes fc)
        {
            string retVal = "-1";
            ...
            {
                //System.Diagnostics.Debug.WriteLine("Defocus: " + SearchForInOuterHtml);
                string temp = element.OuterHtml.ToUpper();
                System.Diagnostics.Debug.WriteLine("TEMP: " + temp);
                if (temp.Contains(whatitscalled))
                {
