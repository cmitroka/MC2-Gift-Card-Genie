...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "giftCardInput", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "pinInput", txtCardPIN.Text);
                if (OK == "1")
                {
            ...
            else
            {
                ...
                balanceResult = GetBalance("Balance$", "Check", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("We did not recognize the following Gift Card number"))
                    {
                        txtCardBalance.Text = "InvGC";
                    ...
                    // 
                    else if (test.Contains("We did not recognize the security code"))
                    {
                        txtCardBalance.Text = "PINPtob";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The PIN is wrong, gift card number seems OK.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
