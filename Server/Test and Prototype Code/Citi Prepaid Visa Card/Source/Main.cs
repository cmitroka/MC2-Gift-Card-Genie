...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "username");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "username", txtLogin.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "password", txtPassword.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "login", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "method1");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "method1");
                if (OK == "-1")
                {
                    //if this exists, go to the next instruction and fill out the info
                    OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ecomPaymentCardNumber");
                }
                else
                {
                    IE.Navigate2(txtBaseURL.Text);
            ...
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "ecomPaymentCardNumber");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "method1");
                if (OK == "1")
                {
                    IE.Navigate2(txtBaseURL.Text);
                    Instruction--;
                    HandleInstruction(OK);//postalCodeNew
                    return;
                }
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ecomPaymentCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ecomPaymentCardVerification", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "register", "");
                HandleInstruction(OK);//postalCodeNew
            }
            else if (Instruction == 5)
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "postalCodeNew");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK=DoHandleNeedsMoreInfoRqRs("Whats the Postal Code?");
                HandleInstruction(OK);//postalCodeNew
            }
            else if (Instruction == 6)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "postalCodeNew", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "register", "");
                HandleInstruction(OK);//postalCodeNew
            }
            else if (Instruction == 7)
            {
                ...
                if (test.Contains("USD"))
                {
                    OK = "1";
                    HandleInstruction(OK);
                    return;
                }
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "termsConditionAccepted", "");
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "submitBtn", "");
                HandleInstruction(OK);//postalCodeNew
            }
            else
            {
                ...
                balanceResult = GetBalance("Balance: USD", "Help", test);
                if (balanceResult == "")
                {
                    OK = "-1";
        ...
        private void DefinetlyExitAppWrs(string reason, string message)
        {
            ...
            SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "We're still trying to get this right, seems either the data entered or our lookup engine failed.  We'll look into it.", ad.RsPathAndFileToWrite);
            ApplicationExit();
        }
