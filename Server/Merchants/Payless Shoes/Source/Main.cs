...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, "captcha.aspx");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardNum", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "captchaText", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.img, HTMLEnumAttributes.id, "checkBalance");
                HandleInstruction(OK);
            }
            else
            {
                IHTMLDocument2 temp=GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1);
                string test = GCGMethods.GetHTMLFromWebObject(temp);
                test = GCGMethods.GetPlainTextFromHTML(test);
                //string test = DoManualCopyAndPaste();
                //GCGMethods.WriteFile("C:\\GetBalance.txt", test, true);
                string balanceResult = GetBalance("Balance:", "check", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
