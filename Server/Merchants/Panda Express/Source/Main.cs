...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, ".jpg");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "card_number", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "security", txtCAPTCHAAnswer.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.div, HTMLEnumAttributes.InnerText, "Check Balance", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance(l4 + " is", ". For", test);
                if (balanceResult == "")
                {
                    //SpecificRetryCnt++;
