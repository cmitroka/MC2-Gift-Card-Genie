...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "cpkstore.wgiftcard.com");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = DoHandleCAPTCHARqRs("cpkstore.wgiftcard.com", 999);
                HandleInstruction(OK);
            }

            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "card_number", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "security", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.classs, "tw_btn", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance(" is ", ".", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("I'm sorry. It looks like you have reached your request limit. Please try again tomorrow."))
                    {
                        txtCardBalance.Text = "InvCN";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Sorry, we've reached our request limit for CPK. Please try again tomorrow.", ad.RsPathAndFileToWrite);
                        OK = "1";
                        IE.Quit();
                    }
                    else if (test.Contains("Please type the characters you see in the image "))
                    {
                        txtCardBalance.Text = "InvP";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The CAPTCHA was wrong, please try again.", ad.RsPathAndFileToWrite);
                        OK = "1";
                        IE.Quit();
                    }
        ...
        private string DoHandleCAPTCHARqRs(string ImgSrcOfCaptcha, int FrameItsIn)
        {
            ...
            {
                OK = copyImageToClipBoard(ImgSrcOfCaptcha);
                //OK = "1";
            }
            else
            {
