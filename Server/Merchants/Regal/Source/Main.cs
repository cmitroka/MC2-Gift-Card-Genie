...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%main_menu_03.gif", "");
            ...
            else if (Instruction == 4)
            {
                ...
                if (OK == "1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.value, "Go!", "");
                }
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("current balance:", "Look Up", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("Invalid information entered, please try again."))
                    {
                        txtCardBalance.Text = "Captcha";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Card # seems OK, CAPTCHA was wrong - try again.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
                    else if (test.Contains("Invalid information entered, please go back and try again."))
                    {
                        txtCardBalance.Text = "Inv";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The Card # seems invalid. Please check and try again.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
