...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.OuterHtml, "Check Balance");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                DoGCGDelay(3,true);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.OuterHtml, "Check Balance", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1,4);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "accountNumber", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "accessCode", txtCardPIN.Text);
                if (OK == "1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "checkGCBalance", "");
                }
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("$", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "An error occurred while looking up the gift card balance.";
                    ...
                    if (test.Contains(VendErr1))
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The Card Number and/or PIN seem wrong.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
