...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleCAPTCHARqRs("GetImgText.ashx");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "ctl03_MainContentArea_ctl00_ctl00_txtGiftCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "ctl03_MainContentArea_ctl00_ctl00_txtCaptcha", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.id, "ctl03_MainContentArea_ctl00_ctl00_imgCheckBalance", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                OK = DoGCGDelay(50, true); 
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("Balance:", "Continue", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "The characters you entered must match those in the image above.";
                    ...
                    if (test.Contains(VendErr1))
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The characters you entered didn't match the image.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
