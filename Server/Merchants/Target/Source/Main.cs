...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "gcCardNumber", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "gcAccessNumber", txtCardPIN.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.id, "giftcard-Save","");
            ...
            else
            {
                ...
                balanceResult = GetBalance("remaining balance:", "date", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("GiftCard information entered is either invalid or incomplete."))
                    {
                        txtCardBalance.Text = "Inv";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "GiftCard information entered is either invalid or incomplete.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
