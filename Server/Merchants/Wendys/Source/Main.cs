...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "email", "temp1@mc2techservices.com");
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "pw", "Temppass1");
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%submit_btn.png", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.OuterHtml, "Check My Card Balance", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, "securebmp.py?");
                HandleInstruction(OK);
            }
            else if (Instruction == 5)
            {
                FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1, 0);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "cardnum", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "cert_pin", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "code", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "balance_check", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("Current Balance", "USD", test);
                if (balanceResult == "")
                {
                    //SpecificRetryCnt++;
            ...
            else if (GetCAPTCHACnt == 3)
            {
                if (CAPTCHAFound == true)
                {
                    try
                    {
                        ...
                        DoGCGDelay(5);
                        bmp = GCGMethods.CaptureRegionAsBMP(220, 710, 200, 45);  //220, 753, 200, 40
