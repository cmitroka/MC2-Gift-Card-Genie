    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            else if (Instruction == 2)
            {
                ...
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input,  HTMLEnumAttributes.name, "biCardNumber", txtCardNumber.Text+txtCardPIN.Text);
                //GCGMethods.SimInput(webBrowser1, GCGCommon.HTMLEnumTagNames.input, CJMUtilities.HTMLEnumAttributes.name, "commit");
                //GCGMethods.SimInput(webBrowser1, GCGCommon.HTMLEnumTagNames.button, CJMUtilities.HTMLEnumAttributes.value, "<SPAN>Check Balance</SPAN>");
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.InnerText, "Check Balance");
                DoDelay(5, true);
                //CheckForPageComplete(2);
            }
            else
            {
                ...
                string balanceResult = GetBalance("Your gift card balance is:", "gift", test);
                if (balanceResult == "")
                {
                    RetryCnt++;
                    if (RetryCnt < 3)
