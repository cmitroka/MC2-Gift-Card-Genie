        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                else if (Instruction == 4)
                {
                    ...
                    OK = GCGMethods.ElementExists(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "TextBoxCardNumber");
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    }
                    OK = SetFocusOnElement("TextBoxCardNumber", GCGMethods.ByNameOrID.Id, -1);
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
