...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "Card.Number", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "Card.Pin", txtCardPIN.Text);
                if (OK != "-1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.value, "Check Balance", "");
                    if (OK == "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.value, "Check My Balance", "");
                    if (OK == "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.value, "CHECK_MY_BALANCE", "");
                }
                if (OK == "1")
                {
            ...
            else
            {
                ...
                if (balanceResult == "") balanceResult = GetBalance("Card Balance", "as", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "That card number or security code is invalid.";
