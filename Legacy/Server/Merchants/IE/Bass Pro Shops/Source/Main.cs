...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                else if (Instruction == 3)
                {
                    ...
                    //OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Za, GCGMethods.HTMLAttributes.ZouterHtml, "Check your balance", "");
                    //HandleInstruction(OK);
                }
                else if (Instruction == 4)
                {
                    IHTMLDocument2 x = GCGMethods.ConvertIEToIHTMLDocument2(IE, -1);
                    OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "gCardNumberID_1", txtCardNumber.Text);
                    if (OK != "-1") OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "gCardPINID_1", txtCardPIN.Text);
                    if (OK != "-1") OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zname, "checkGCBalance", "");
                    HandleInstruction(OK);
                }
                else if (Instruction == 999)
                {
                    ...
                    DoHandleTyper(txtCardNumber.Text, 0);
                    DoHandleTyper("{TAB}" + txtCardPIN.Text + "{TAB}{ENTER}");
                    DoGCGDelay(15, true);
                    HandleInstruction(OK);
                }
                else
                {
                    ...
                    string roughParse1 = GCGMethods.RoughExtract("id=\"gcResults\">", "</tbody>", testi);
                    string roughParse2 = GCGMethods.RoughExtract("id=\"gcResults\">", "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", roughParse1);
                    balanceResult = GetBalance("id=\"gcResults\">", "</td>", roughParse2);
                    if (balanceResult == "")
                    {
                        SpecificRetryCnt++;
                        ...
                        if (SpecificRetryCnt >= 10)
                        {
                            ...
                            SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "We couldn't get a balance; please verify the numbers are correct and try again.", ad.RsPathAndFileToWrite);
                            OK = "1";
                        }
                    }
                    else
        ...
        private void MouseMove(int pXPos, int pYPos)
        {
            ...
            */        
            //Cursor.Position = new Point(pXPos, pYPos);
