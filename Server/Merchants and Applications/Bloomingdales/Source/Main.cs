...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%captcha");
                if (OK == "1")
                {
                    OK = CAPTCHALib.DoHandleCAPTCHARqRs(this, 487, 646);
                }
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                string EntireNum = txtCardNumber.Text + txtCardPIN.Text;
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "GiftCardNumber1", EntireNum);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "SecurityWord", txtCAPTCHAAnswer.Text);
                if (OK == "1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "CHECK_BUTTON", "");
                }
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("Gift Card 1:", "Gift Card 2", test);
                if (balanceResult == "")
                {
                    OK = "-1";
