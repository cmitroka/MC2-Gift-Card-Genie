    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                {
                    int td = GCGMethods.FindWhatFrameItsIn(IE, "gift-card-number");
                    IHTMLDocument2 x = GCGMethods.ConvertIEToIHTMLDocument2(IE, td);
                    //SetForegroundWindowByHWND(IE.HWND);
                    OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "gift-card-number", txtCardNumber.Text, 1);
                    if (OK != "-1") OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "gift-card-pin", txtCardPIN.Text, 1);
                    if (OK != "-1") OK = GCGMethods.SimInput(x, GCGMethods.HTMLTagNames.Zbutton, GCGMethods.HTMLAttributes.Zid, "btn-check-balance", "", 1);
                ...
                else
                {
                    ...
                    balanceResult = GetBalance("has a remaining balance of $", ".</div>", testi);
                    if (balanceResult == "")
                    {
                        SpecificRetryCnt++;
