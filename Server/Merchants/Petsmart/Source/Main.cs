...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.innerHTML, "online");
                if (OK != "-1")
                {
                    OK = GCGMethods.ModifyHTML(FrameDoc, "a", "innerHTML", "online", "<a href=\"/coreg/index.jsp?step=giftcardBalance&reset=true\" class=\"pagelink\">online</a>");
                }
            ...
            else if (Instruction == 3)
            {
                ...
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.innerHTML, "online","");
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "accountNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "pin", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%gcbal_balance.gif", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                test = GCGMethods.RoughExtract("Gift Card Balance as of", "return", test);
                if (balanceResult == "") balanceResult = GetBalance(":", "WINDOW", test);
                if (balanceResult == "")
                {
                    OK = "-1";
