        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                else if (Instruction == 5)
                {
                    ...
                    OK = GCGMethods.ElementExists(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zname, "ctl00$ContentPlaceHolder1$txtGiftCardNumber");
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    }
                    OK = SetFocusOnElement("ctl00$ContentPlaceHolder1$txtGiftCardNumber", GCGMethods.ByNameOrID.Name, frame);
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    }
                    //DoUI.DoMouseClick(224, 333);
                    DoHandleTyper(txtCardNumber.Text + "{TAB}" + txtCardPIN.Text + "{TAB}{TAB}{TAB}{TAB}" + txtCAPTCHAAnswer.Text + "{TAB}{ENTER}");
                ...
                else
                {
                    ...
                    balanceResult = GetBalance("Gift Card is currently:", "div", testi);
                    if (balanceResult == "")
                    {
                        OK = "-1";
