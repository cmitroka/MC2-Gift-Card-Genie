...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK=DoHandleCAPTCHARqRs("image?");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                PMCMacros.DoMacroBWW(this);
                OK ="1";
                //webBrowser1.Navigate("javascript:document.getElementById('ctl00_MainContent_GiftCardNumber').focus();");
                //OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "ctl00_MainContent_GiftCardNumber", txtCardNumber.Text);
            ...
            {
                webBrowser1.Navigate("javascript:document.getElementById('recaptcha_response_field').focus();");
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.id, "giftCardFormSubmit", "");
                HandleInstruction(OK);
            }
            */ 
            else
            {
                ...
                balanceResult = GetBalance("Balance:", "Date", test);                
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "The PIN is invalid.";
                    string VendErr2 = "Never Find this...";
                ...
                {
                    if (balanceResult == "$0.00")
                    {
                        if (testhtml.Contains("Credit"))
                        {
                        ...
                        {
                            txtCardBalance.Text = "VE2";
                            SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "It seems the card info is invalid.", ad.RsPathAndFileToWrite);
                            OK = "1";
                        }
                    }
                    else
