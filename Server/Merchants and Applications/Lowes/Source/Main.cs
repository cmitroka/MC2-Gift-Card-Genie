    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            else if (Instruction == 2)
            {
                ...
                int CAPTCHAFI = GCGMethods.FindWhatFrameItsIn(IE, "jsessionid");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, CAPTCHAFI);
                OK=DoHandleCAPTCHARqRs("jsessionid", 999);
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {

                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "check_your_balance_CTA.PNG");
                ...
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "mockpass", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "realpass", txtCardPIN.Text);
                ...
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.classs, "btninput", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("$", "Check", test);
                if (balanceResult == "")
                {
                    OK = "-1";
        ...
        private string copyImageToClipBoard(string containsimgname)
        {
            ...
            foreach (mshtml.IHTMLImgElement img in doc.images)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    {
                        GCGMethods.WriteTextBoxLog(txtLog, "SaveCAPTCHA fail " + i.ToString());
                        retVal = "-1";
                    }
