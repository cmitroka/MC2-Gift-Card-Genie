...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleCAPTCHARqRs("recaptcha/api/image");
                //OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%btn_check_gift_card_bal.gif", "");
                //OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "dwfrm_billinggiftcert_giftCertCode");
                //btn_check_gift_card_bal.gif
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "number", txtCardNumber.Text);
                if (OK=="1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "pin", txtCardPIN.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "recaptcha_response_field", txtCAPTCHAAnswer.Text);
                if (OK == "1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.value, " ", "");
                }
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("has a current balance of ", "History", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("The gift card number you entered is not a valid"))
                    {
                        txtCardBalance.Text = "Inv";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The gift card number you entered is not a valid.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
                    else if (test.Contains("We encountered a problem"))
                    {
                        txtCardBalance.Text = "Prob";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "We encountered a problem with this gift card transaction. For gift card questions, please call 1-877 PLACE USA.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
        ...
        private void Main_Load(object sender, EventArgs e)
        {
            ...
            AuxFunctions.LoadSettings(this);
            System.Diagnostics.Debug.WriteLine(SpecificRetryCntMax);
            AuxFunctions.LoadCommonConfig(this);
            ...
            string result = AuxFunctions.LoadCLIrqFile(this);
            if (result != "1")
            {
        ...
        {
            AuxFunctions.LoadCLIrqFile(this);
        }
