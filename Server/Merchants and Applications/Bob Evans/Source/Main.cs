...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleCAPTCHARqRs("jpg");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                string cardnum = txtCardNumber.Text.Substring(0, 16);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "card_number", cardnum);
            ...
            else
            {
                ...
                balanceResult = GetBalance("$", "if", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
