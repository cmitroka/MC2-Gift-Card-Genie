...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "email_returning", txtLogin.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "password", txtPassword.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.name, "sign_in", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK=GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "site-search-field");
                if (OK != "-1") webBrowser1.Navigate("https://www.sephora.com/profile/MyAccount/PaymentMethods/paymentMethods.jsp");
            }
            else if (Instruction == 4)
            {
                webBrowser1.Navigate("javascript:document.getElementById('gc-number').focus();");
                string OK2 = DoGCGDelay(3, true);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gc-number", txtCardNumber.Text);
                if (OK != "-1") GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gc-pin", txtCardPIN.Text);
                //if (OK != "-1") GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "gc-pin", txtCardPIN.Text);
                //if (OK != "-1") webBrowser1.Navigate("javascript:document.getElementByName('check').focus();");
                //if (OK != "-1") GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.name, "check", "");
                HandleInstruction(OK);
            }
            else if (Instruction == 5)
            {
                PMCMacros.DoMacroSephora(this);
            }
            else
            {
                ...
                if (balanceResult == "") balanceResult = GetBalance("Balance = ", "USD", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("Sorry, the card number you entered is either closed or invalid"))
                    {
                        txtCardBalance.Text = "Inv";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Sorry, the card number you entered is either closed or invalid.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
                    else if (test.Contains("The PIN is invalid"))
                    {
                        txtCardBalance.Text = "InvP";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The PIN is invalid. Please check your PIN and try again.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
                    else if (test.Contains("Please enter a PIN Number"))
                    {
                        txtCardBalance.Text = "InvP";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Please enter a PIN Number.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
