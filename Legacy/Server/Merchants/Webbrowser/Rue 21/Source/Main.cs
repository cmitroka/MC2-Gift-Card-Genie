...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "ctl00_GiftCardOrderContent_txtCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "ctl00_GiftCardOrderContent_ucCaptcha_tbHipAlphaNumeric", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$GiftCardOrderContent$btnContinue", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                if (balanceResult == "") balanceResult = GetBalance(" is ", "click", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "We apologize, there was an error during retrieving the card balance. Please verify the card number";
                    string VendErr2 = "The characters which you typed did not match with the characters in the picture";
                    ...
                    if (test.Contains(VendErr1))
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The online lookup for Rue 21 is having issues.  Hopefully they'll be fixed soon, maybe try again in a week...", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
