...
namespace AppWebService
{
    public class SQLHelper
    {
        ...
        public SQLHelper(string mdbtouse)
        {
            GetAndOpenOleDbConnection(mdbtouse);
        }
        ...
        private OleDbConnection GetAndOpenOleDbConnection(string mdbtouse)
        {
            ...
            try
            {
                aConnection = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + MDBPath + "App_Data\\" + mdbtouse);
                aConnection.Open();
            }
            catch (Exception ex)
            {
