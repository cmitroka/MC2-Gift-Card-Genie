...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.InnerText, "here");
                //DoDelay(40);
            ...
            {
                OK = GCGMethods.SimInput2(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.id, "cardNoH", txtCardNumber.Text);
                OK = GCGMethods.SimInput2(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.id, "inCaptchaChars", txtCAPTCHAAnswer.Text);
                OK = GCGMethods.SimInput2(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.id, "submit", "");
                HandleInstruction(OK);
            }
            else
            {
                string test = GCGMethods.GetFrameDocumentText(webBrowser1);
                //string test = DoManualCopyAndPaste();
                //GCGMethods.WriteFile("C:\\GetBalance.txt", test, true);
                string balanceResult = GetBalance("current balance:", "lookup", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
