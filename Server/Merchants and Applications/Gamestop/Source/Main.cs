...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GrabCorrectPageOrFrame("Check your Balance");
                OK = WebpageLib00.ElemLinkEnvoke(IE,"giftCardBalance.jsp");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {

                OK = WebpageLib00.CAPTCHAGetImage(IE, "kaptcha");
                if (OK=="1") OK= DoHandleCAPTCHARqRs();
                HandleInstruction(OK);
            }
            else if (Instruction == 4)
            {
                OK = WebpageLib00.ElemInputEnvoke(IE, "giftNumEntry", txtCardNumber.Text);
                if (OK != "-1") OK = WebpageLib00.ElemInputEnvoke(IE, "pinNumEntry", txtCardPIN.Text);
                if (OK != "-1") OK = WebpageLib00.ElemInputEnvoke(IE, "captchaEntered", txtCAPTCHAAnswer.Text);
                //if (OK != "-1") OK = WebpageLib00.ElemInputEnvoke(IE, "NAME", "checkBalance", "");
                if (OK != "-1") OK = WebpageLib00.ElemInputEnvoke(IE, "gcBal_checkSubmit", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                string testi = FrameDoc.body.innerHTML;
                string testo = FrameDoc.body.outerHTML;
                ...
                */
                balanceResult = GetBalance("gcBalance\">", "</span>.", testi);
                if (balanceResult == "")
                {
                    SpecificRetryCnt++;
                    ...
                    if (SpecificRetryCnt>=10)
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "There was a problem with the CAPTCHA you sent.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
