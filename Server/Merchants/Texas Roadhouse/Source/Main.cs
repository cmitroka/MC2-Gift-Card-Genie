...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        {
            /*
<img src="captcha.jpg" id="kaptchaImage" width="101" height="35" />
<input id="giftCardBalInquiryForm:cardno" type="text" name="giftCardBalInquiryForm:cardno" class="inputfield" maxlength="19" size="19" />
<input id="giftCardBalInquiryForm:pinno" type="password" name="giftCardBalInquiryForm:pinno" value="" maxlength="4" size="4" class="inputfield" />
<input id="giftCardBalInquiryForm:kaptchafield" type="text" name="giftCardBalInquiryForm:kaptchafield" class="inputfield" maxlength="5" size="5" />
<input id="giftCardBalInquiryForm:submitButton" type="submit" name="giftCardBalInquiryForm:submitButton" value="Lookup" class="formButton" />
             */
            ...
            {
                DoHandleRqRs(GCTypes.GCCAPTCHA, "captcha.jpg");
            }
            else if (Instruction == 3)
            {
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardBalInquiryForm:cardno", txtCardNumber.Text);
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardBalInquiryForm:pinno", txtCardPIN.Text);
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardBalInquiryForm:kaptchafield", txtCAPTCHAAnswer.Text);
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardBalInquiryForm:submitButton");
            }
            else
            {
                //Defocus(CJMUtilities.HTMLEnumAttributes.name, "Txtgcnum");
                string test = GetWBDocumentText();
                //GCGMethods.WriteFile("C:\\GetBalance.txt", test, true);
                string balanceResult = GetBalance("Current Balance", "Close", test);
                if (balanceResult == "")
                {
                    RetryCnt++;
                    if (RetryCnt < 3)
        ...
        private void tmrPageDone_Tick(object sender, EventArgs e)
        {
            ...
            if (WBPageDoneCnt >= 4)
            {
                ...
                if (tmrDelay.Enabled == false) ProcessInstructions();
            }
        }
        ...
        private string GetWBDocumentText()
        {
            string retVal="";
            string temp = "";
            try
            {
                temp=GCGCommon.SupportMethods.ReturnTextOnlyFromHTML(webBrowser1.DocumentText);
            }
            catch (Exception)
            {
            }
