...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.ElementExists(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.innerHTML, "VIEW GIFT CARD BALANCE");
                if (OK != "-1") OK = GCGMethods.ModifyHTML(FrameDoc, "a", "innerHTML", "VIEW GIFT CARD BALANCE", "<a class=\"gc-balance\" href=\"https://wbiprod.storedvalue.com/WBI/lookupservlet?language=en\">VIEW GIFT CARD BALANCE</a>");
                if (OK != "-1") DoGCGDelay(3, true);
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {

                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.a, HTMLEnumAttributes.href, "*%lookupservlet","");
                HandleInstruction(OK);
            }
            ...
            else if (Instruction == 5)
            {
                ...
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.value, "Lookup", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                if (balanceResult == "") balanceResult = GetBalance("balance of", "on", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "Invalid information entered, please try again.";
                    string VendErr2 = "Please enter a valid Gift Card and PIN number";
                    ...
                    else if (test.Contains(VendErr2))
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), VendErr2 + ", one (or both) are incorrect.  You did answer the challenge response correctly.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
