    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                {
                    OK = DoHandleCAPTCHARqRs("image?c="); //image?c= //kaptcha?
                    HandleInstruction(OK);
                }
                else if (Instruction == 5)
                {
                    ...
                    IHTMLDocument2 x = GCGMethods.ConvertIEToIHTMLDocument2(IE, -1);
                    OK = GCGMethods.ElementExists(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "ctl00_GiftCardContent_ctrBalanceInquiry_tbCardNumber");
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    }
                    OK = SetFocusOnElement("ctl00_GiftCardContent_ctrBalanceInquiry_tbCardNumber", GCGMethods.ByNameOrID.Id, -1);
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    ...
                    DoHandleTyper("{TAB}none@gmail.com{TAB}{TAB}" + txtCAPTCHAAnswer.Text + "{TAB}{ENTER}");
                    DoGCGDelay(15, true);
                    HandleInstruction(OK);
                }
                else
                {
                    ...
                    //GCGMethods.WriteFile("C:\\test.txt", testi, true);
                    //GCGMethods.WriteFile("C:\\testi.txt", testi, true);
                    //GCGMethods.WriteFile("C:\\testo.txt", testo, true);
                    balanceResult = GetBalance("_lblBalance", "span", testi);
                    if (balanceResult == "")
                    {
                        SpecificRetryCnt++;
