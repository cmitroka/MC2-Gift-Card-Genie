...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.img, HTMLEnumAttributes.src, "*%b_checkbalance", "");
                HandleInstruction(OK);

            }
            else if (Instruction == 3)
            {
                FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1,1);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "giftCardNumber", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "pin", txtCardPIN.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "b_checkbalance", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("balance is:", "check", test);
                if (balanceResult == "")
                {
                    //SpecificRetryCnt++;
        ...
        private void PlayMacro(string Script)
        {
            ...
            sw.Close();
            string[] gfpp = GCGCommon.SupportMethods.GetFilePathPieces(Application.ExecutablePath);
            string TruePath = gfpp[1];
            string GCGMacroPlayerLoc = gfpp[1] + "\\GCGMacroPlayer.exe";
            TruePath = "C:\\";
            if (!File.Exists(GCGMacroPlayerLoc)) GCGMacroPlayerLoc = "C:\\GCGMacroPlayer.exe";
            GCGMethods.WriteTextBoxLog(txtLog, "Launching " + GCGMacroPlayerLoc + " with arg " + completetemp);
        ...
        private void DoMacro0()
        {
            ...
            string FileToUse = GCGCommon.PMC.WriteMacro(txtRqRsPath.Text, "" +
                "Pause,1000~!~" +
                "WinMove,0,0,800,700,T.G.I. Friday's® Gift Card Balance Lookup - Windows Internet Explorer~!~" +
                "Pause,1000~!~" +
                "Move,479,429~!~" +
                "RightClick~!~" +
                "Pause,1000~!~" +
        ...
        private void DoMacro1()
        {
            ...
            string FileToUse = GCGCommon.PMC.WriteMacro(txtRqRsPath.Text, "" +
                "Pause,1000~!~" +
                "WinActivate,T.G.I. Friday's® Gift Card Balance Lookup - Windows Internet Explorer~!~" +
                "Pause,1000~!~" +
                "Move,450,350~!~" +
                "LeftClick~!~" +
                "Pause,100~!~" +
