...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        {
            /*
<input type="text" name="card_number" value="" id="card_number" maxlength="30" size="25">
<input type="text" name="security" value="" id="security" maxlength="30" size="25">
<div class="tw_btn" onclick="document.tw_form.submit();" style="display:inline-block;">Check Balance</div>
             */
            ...
            {
                DoHandleRqRs(GCTypes.GCCAPTCHA, "wgiftcard.com/public/tmp");
            }
            else if (Instruction == 3)
            {
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "card_number", txtCardNumber.Text);
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "security", txtCAPTCHAAnswer.Text);
                GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.div, HTMLEnumAttributes.InnerText, "Check Balance");
            }
            else
            {
                ...
                string balanceResult = GetBalance("is", "for", test);
                if (balanceResult == "")
                {
                    RetryCnt++;
                    if (RetryCnt < 3)
        ...
        private void tmrGetCAPTCHA_Tick(object sender, EventArgs e)
        {
            ...
            if (GetCAPTCHACnt == 1)
            {
                try
                {
                    ...
                    {
                        if (img.src.Contains(CAPTCHAName))
                        {
