...
//                DoUI.SetForegroundWindowByHWND(MainWindowHWND);
                DoGCGDelay(3, true);
                DoUI.DoMouseClick(320, 324);
                DoHandleTyper(txtCardNumber.Text,0);
                DoHandleTyper("{TAB}");
                DoHandleTyper(txtCardPIN.Text + "{TAB}{ENTER}");
                HandleInstruction("1");
            }
            else
            {
                ...
                string balanceResult = GetBalance(temp, "check", test);
                if (balanceResult == "")
                {
                    SpecificRetryCnt++;
