    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                {
                    OK = DoHandleCAPTCHARqRs("ImageHipChallenge.aspx"); //image?c= //kaptcha?
                    HandleInstruction(OK);
                }
                else if (Instruction == 5)
                {
                    ...
                    OK = GCGMethods.ElementExists(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zname, "ctl00$BodyContent$ucCaptcha$tbHipAlphaNumeric");
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                    }
                    OK = SetFocusOnElement("ctl00$BodyContent$ucCaptcha$tbHipAlphaNumeric", GCGMethods.ByNameOrID.Name, frame);
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
                ...
                else
                {
                    ...
                    balanceResult = GetBalance("BodyContent_lblBalance", "span", testi);
                    if (balanceResult == "")
                    {
                        OK = "-1";
