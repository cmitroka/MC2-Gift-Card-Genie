...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        const string useragent = "Mozilla/5.0+(compatible;+MSIE+10.0;+Windows+NT+6.1;+WOW64;+Trident/6.1)";
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$MainContent$txtGiftCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$MainContent$txtGiftCardPin", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$MainContent$imgCheckGiftCard", "");
            ...
            else
            {
                ...
                if (balanceResult == "") balanceResult = GetBalance("Your Balance is :", "Check", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    if (test.Contains("Please enter a valid Gift Card and PIN number"))
                    {
                        txtCardBalance.Text = "Inv";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Please enter a valid Gift Card and PIN number. Please check them and try again.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
        ...
        {
            //ExtendedWebBrowser ewb = new ExtendedWebBrowser();
            //webBrowser1 = ewb;
            //ewb.UserAgent = "Mozilla/5.0+(compatible;+MSIE+10.0;+Windows+NT+6.1;+WOW64;+Trident/6.1)";
            //useragent = "User-Agent: Mozilla/5.0+(compatible;+MSIE+10.0;+Windows+NT+6.1;+WOW64;+Trident/6.1)";
            //UrlMkSetSessionOption(URLMON_OPTION_USERAGENT, useragent, useragent.Length, 0);
            webBrowser1.Navigate(txtBaseURL.Text,null,null,useragent);
        }
