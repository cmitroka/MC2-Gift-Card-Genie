        ...
        private void ProcessInstructions()
        {
            try
            {
                ...
                else if (Instruction == 3)
                {
                    ...
                    if (OK == "1") SpecificRetryCnt = 0;
                    SpecificRetryCnt++;
                    GCGMethods.WriteTextBoxLog(txtLog, "SpecificRetryCnt: " + SpecificRetryCnt.ToString());
                    if (SpecificRetryCnt >= 5)
                    {
                        txtCardBalance.Text = "Cap Error";
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "Lowe's is having an issue with their online lookup, please try again later.", ad.RsPathAndFileToWrite);
                ...
                else if (Instruction == 4)
                {
                    ...
                    OK = GCGMethods.ElementExists(x, GCGMethods.HTMLTagNames.Zinput, GCGMethods.HTMLAttributes.Zid, "search-query");
                    if (OK == "-1")
                    {
                        HandleInstruction(OK);
                        return;
