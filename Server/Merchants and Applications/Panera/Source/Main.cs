...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                int SubFrame = GCGMethods.FindWhatFrameItsIn(IE, "ImageHipChallenge.aspx");
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = DoHandleCAPTCHARqRs("ImageHipChallenge.aspx", 999);
            ...
            else if (Instruction == 3)
            {
                ...
                FrameDoc = GCGMethods.ConvertIEToIHTMLDocument(IE, SubFrame);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$BodyContent$tbCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$BodyContent$ucCaptcha$tbHipAlphaNumeric", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.button, HTMLEnumAttributes.id, "ctl00_BodyContent_cont", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance(l4, "Summary", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                ...
                else
                {
                    if (balanceResult == "$0.00")
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "They're reporting $0, but this can occur if you didn't enter a valid card number.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
                    else
                    {
