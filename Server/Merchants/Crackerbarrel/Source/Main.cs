...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, "CaptchaHandler.ashx?=");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "rightsidebar_1$txtCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "rightsidebar_1$CaptchaText", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput(webBrowser1, HTMLEnumTagNames.a, HTMLEnumAttributes.id, "rightsidebar_1_lnkCheckBalance", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance(txtCardNumber.Text, "Check", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
        ...
        {
            string retval = "";
            GetBalance(startTag, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", allText);
            return retval;
        }
        ...
        private string ChangeFocus(string SearchForInOuterHtml, GCGCommon.HTMLEnumTagNames htmltype, FocusTypes fc)
        {
            ...
            HtmlElementCollection col = webBrowser1.Document.GetElementsByTagName(a);  //can be hr, or input, or whatever
