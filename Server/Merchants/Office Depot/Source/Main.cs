...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "storedValueCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "storedValueCardPin", txtCardPIN.Text);
                //if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.OuterHtml, "Submit", "");
                if (OK == "1")
                {
                    DoMacro1();
                }
            ...
            else
            {
                ...
                balanceResult = GetBalance("There is", "remaining ", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
