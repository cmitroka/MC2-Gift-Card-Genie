...
namespace DVB
{
    ...
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleRqRs(GCTypes.GCCAPTCHA, "image?c");
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                IHTMLDocument2 FrameDoc = GCGMethods.ConvertWebBrowserToIHTMLDocument2(webBrowser1);
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "giftCardNumberInputField", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "giftCardPINInputField", txtCardPIN.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "recaptcha_response_field", txtCAPTCHAAnswer.Text);
            ...
            else
            {
                ...
                test= test.Replace(" AM", "XXXXXXXXXX");
                test=test.Replace(" PM", "XXXXXXXXXX");
                //GCGMethods.WriteFile("C:\\GetBalance.txt", test, true);
                balanceResult = GetBalance("XXXXXXXXXX", "-->", test);
                if (balanceResult == "")
                {
                    OK = "-1";
                    HandleInstruction(OK);
