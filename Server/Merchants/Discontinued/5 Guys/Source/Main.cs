...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$GiftCardContent$txtCardNumber", txtCardNumber.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$GiftCardContent$ucCaptcha$tbHipAlphaNumeric", txtCAPTCHAAnswer.Text);
                if (OK != "-1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "ctl00$GiftCardContent$btnSearch", "");
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("YOU HAVE", "remaining", test); 
                if (balanceResult == "")
                {
                    OK = "-1";
                    string VendErr1 = "The characters which you typed did not match with the characters in the picture.Card";
                    ...
                    if (test.Contains(VendErr1))
                    {
                        ...
                        SupportMethods.WriteResponseFile(GCGCommon.GCTypes.GCCUSTOM.ToString(), "The characters which you typed did not match with the characters in the picture.", ad.RsPathAndFileToWrite);
                        OK = "1";
                    }
