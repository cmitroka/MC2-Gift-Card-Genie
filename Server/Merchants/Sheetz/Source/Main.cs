...
namespace DVB
{
    public partial class Main : Form
    {
        ...
        private void ProcessInstructions()
        {
            ...
            {
                OK = DoHandleCAPTCHARqRs("Captcha.jpg",0);
                HandleInstruction(OK);
            }
            else if (Instruction == 3)
            {
                OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "captcha", txtCAPTCHAAnswer.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "number", txtCardNumber.Text);
                if (OK == "1") OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.id, "pin", txtCardPIN.Text);
                if (OK == "1")
                {
                    OK = GCGMethods.SimInput2(FrameDoc, HTMLEnumTagNames.input, HTMLEnumAttributes.name, "submit", "");
                }
                HandleInstruction(OK);
            }
            else
            {
                ...
                balanceResult = GetBalance("card is", test);
                if (balanceResult == "")
                {
                    OK = "-1";
